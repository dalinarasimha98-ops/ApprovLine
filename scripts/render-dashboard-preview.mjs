#!/usr/bin/env node
/**
 * Renders the real, live components/dashboard/OrganizationDashboardView.tsx
 * (not a mock) with real data from getDashboardOverview() to static HTML,
 * for visual/responsive verification when no live Clerk session is
 * available (e.g. this repo's own CI/dev sandboxes have no Clerk
 ...
 *     node --import tsx scripts/render-dashboard-preview.mjs \
 *     <organizationId> <userId> <email> <role> <outFile.html> [range]
 */
...
import { OrganizationDashboardView } from '../components/dashboard/OrganizationDashboardView.tsx';
import { getDashboardOverview, resolveDashboardRange } from '../services/dashboard.ts';

const [organizationId, userId, email, role, outFile, rangeKey] = process.argv.slice(2);
if (!organizationId || !userId || !email || !role || !outFile) {
  console.error('Usage: render-dashboard-preview.mjs <organizationId> <userId> <email> <role> <outFile.html> [range]');
  process.exit(1);
}
...
function loadCompiledCss() {
  ...
  try {
    ...
    console.warn('[render-dashboard-preview] no .next/static/css found - run `npm run build` first for real styling.');
    return '';
  }
}

async function main() {
  const range = resolveDashboardRange(rangeKey ?? '30d');
  const overview = await getDashboardOverview({ organizationId, userId, email, role }, range);

  const markup = renderToStaticMarkup(
    React.createElement(
      AppRouterContext.Provider,
      { value: mockRouter },
      React.createElement(OrganizationDashboardView, {
        overview,
...
<title>Organization Dashboard preview</title>
<style>${css}</style>
