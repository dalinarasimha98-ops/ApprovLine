...
function InsightIcon({ type }: { type: ComplianceInsight['type'] }) {
  if (type === 'critical') {
    return (
      <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-al-danger/20">
        <svg className="h-4 w-4 text-al-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  if (type === 'warning') {
    return (
      <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-al-warning/20">
        <svg className="h-4 w-4 text-al-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
      </div>
    );
  ...
  if (type === 'positive') {
    return (
      <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-al-success/20">
        <svg className="h-4 w-4 text-al-success" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  // info
  return (
    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-al-accent-hover/20">
...
function insightBorderColor(type: ComplianceInsight['type']): string {
  if (type === 'critical') return 'border-al-danger/20';
  if (type === 'warning') return 'border-al-warning/20';
  if (type === 'positive') return 'border-al-success/20';
  return 'border-al-accent/20';
}
    ...
    <div className="grid gap-5">
      ...
      <div className="rounded-2xl border border-al-border bg-al-surface p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-al-accent">Compliance Insights AI</p>
            <p className="text-[10px] text-al-text-muted">Rule-based from live data</p>
          </div>
        </div>
              ...
              >
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[10px] text-al-text-muted leading-snug">{insight.description}</p>
                    <Link
                      href={insight.filterHref}
                      className="mt-2 inline-flex items-center gap-1 text-[10px] font-bold text-al-accent hover:text-al-accent transition-colors"
                    >
                      View details
          ...
          {[
            ...
            <div key={label}>
              <div className="flex items-center justify-between gap-1 mb-1">
                <span className="text-[10px] font-semibold text-al-text-muted truncate">{label}</span>
                {pct > 0 && (
