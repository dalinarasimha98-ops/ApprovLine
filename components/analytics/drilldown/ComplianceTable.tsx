...
function RiskBadge({ riskLevel }: { riskLevel: string | null }) {
  ...
  let cls = 'bg-al-success/15 text-al-success border-al-success/20';
  if (level === 'critical') cls = 'bg-red-900/30 text-red-300 border-red-900/40';
  else if (level === 'high') cls = 'bg-al-danger/15 text-al-danger border-al-danger/20';
  else if (level === 'medium') cls = 'bg-al-warning/15 text-al-warning border-al-warning/20';
...
function ComplianceStatusBadge({ score, status }: { score: number; status: string }) {
  if (status?.toUpperCase() === 'PENDING_REVIEW') {
    return (
      <span className="inline-flex items-center rounded-full border border-al-info/20 bg-al-info/100/15 px-2 py-0.5 text-[10px] font-black text-al-info">
        Pending
      </span>
    );
  }
  if (score >= 80) {
    return (
      <span className="inline-flex items-center rounded-full border border-al-success/20 bg-al-success/15 px-2 py-0.5 text-[10px] font-black text-al-success">
        Compliant
      </span>
    );
  }
  if (score >= 60) {
    return (
      <span className="inline-flex items-center rounded-full border border-al-warning/20 bg-al-warning/15 px-2 py-0.5 text-[10px] font-black text-al-warning">
        Needs Attention
      </span>
    );
  }
...
function ScoreBar({ value, color }: { value: number; color: string }) {
  return (
    <div className="flex items-center gap-1.5 min-w-[80px]">
      <div className="h-1.5 flex-1 rounded-full bg-al-border">
        <div
...
function SourceBadge({ source }: { source: string | null }) {
  ...
  const colors: Record<string, string> = {
    ...
    oracle: 'bg-red-600/10 text-al-danger',
        ...
        <thead>
          <tr className="border-b border-al-border">
            {[
              ...
              <th
                key={col}
                className="px-4 py-3 text-[9px] font-black uppercase tracking-[0.15em] text-al-text-muted whitespace-nowrap"
              >
                {col}
              </th>
            ))}
        ...
        <tbody>
          {records.map((record) => {
            ...
            return (
              <tr
                key={record.id}
                className="border-b border-al-border/60 align-middle hover:bg-al-border/30 transition-colors"
              >
                {/* Record ID */}
                <td className="px-4 py-3">
                  <span className="font-mono text-[10px] text-al-accent">
                ...
                <td className="px-4 py-3">
                  <span
                    className="text-[11px] text-al-text-muted"
