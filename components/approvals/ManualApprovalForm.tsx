  ...
  return (
    <form onSubmit={submit} className="grid gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label={`Confidence: ${values.confidenceLevel}%`}><input type="range" min="0" max="100" className="h-11 w-full accent-al-accent" disabled={saving} value={values.confidenceLevel} onChange={(e) => update('confidenceLevel', Number(e.target.value))} /></Field>
      ...
      <label className="flex items-center gap-3 rounded-xl border border-al-border bg-al-surface-sunken p-4 text-sm font-bold text-al-text-secondary"><input type="checkbox" className="h-4 w-4 accent-al-accent" checked={values.secondPersonRequired} disabled={saving} onChange={(e) => update('secondPersonRequired', e.target.checked)} />Require second-person verification</label>
