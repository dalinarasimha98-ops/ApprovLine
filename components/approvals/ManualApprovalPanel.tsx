  ...
  return (
    <section className="grid gap-5 rounded-3xl border border-blue-200 bg-white p-6 shadow-sm" aria-labelledby="manual-approval-heading">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2"><span className={`rounded-full border px-3 py-1.5 text-xs font-black ${statusTone[detail.verificationStatus]}`}>{display(detail.verificationStatus)}</span>{canManage ? <button type="button" onClick={() => setEditing(true)} className="h-10 rounded-xl border border-al-border bg-white px-4 text-sm font-black text-al-text-secondary">Edit record</button> : null}</div>
      </div>
      ...
      {canManage ? <div className="flex flex-wrap gap-3"><button type="button" disabled={working !== null} onClick={requestSuggestions} className="h-11 rounded-xl bg-al-accent px-4 text-sm font-black text-white disabled:opacity-60">{working === 'suggestions' ? 'Searching...' : 'Find supporting evidence'}</button><button type="button" disabled={working !== null} onClick={createConfirmation} className="h-11 rounded-xl border border-al-border bg-white px-4 text-sm font-black text-al-text-secondary disabled:opacity-60">{working === 'confirmation' ? 'Creating...' : 'Request approver confirmation'}</button></div> : null}
