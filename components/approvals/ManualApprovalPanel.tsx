...
const statusTone: Record<string, string> = {
  PENDING_CONFIRMATION: 'border-al-warning/30 bg-al-warning/10 text-al-warning',
  CONFIRMED_BY_APPROVER: 'border-al-success/30 bg-al-success/10 text-al-success',
  DISPUTED: 'border-al-danger/30 bg-al-danger/10 text-rose-800',
  SUPERSEDED: 'border-al-border bg-al-surface-elevated text-al-text-secondary',
};
...
export function ManualApprovalPanel({ approval, detail, evidence, versions, confirmations, canManage, currentUserId, currentUserRole }: {
  ...
  if (editing) return <section className="rounded-2xl border border-al-info/30 bg-al-surface p-6 shadow-sm"><ManualApprovalForm approvalId={approval.id} initial={initial} onCancel={() => setEditing(false)} /></section>;

  return (
    <section className="grid gap-5 rounded-3xl border border-al-info/30 bg-al-surface p-6 shadow-sm" aria-labelledby="manual-approval-heading">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-black uppercase tracking-[0.16em] text-al-accent">Defensible manual evidence</p><h3 id="manual-approval-heading" className="mt-1 text-xl font-black text-al-text">{detail.kind === 'VERBAL' ? 'Verbal approval record' : 'Manually recorded approval'}</h3><p className="mt-2 text-sm leading-6 text-al-text-secondary">Recorded by {detail.recorder?.name ?? detail.recorder?.email ?? 'Unknown recorder'}. This record is never presented as automatically captured evidence.</p></div>
        <div className="flex flex-wrap gap-2"><span className={`rounded-full border px-3 py-1.5 text-xs font-black ${statusTone[detail.verificationStatus]}`}>{display(detail.verificationStatus)}</span>{canManage ? <button type="button" onClick={() => setEditing(true)} className="h-10 rounded-xl border border-al-border bg-al-surface px-4 text-sm font-black text-al-text-secondary">Edit record</button> : null}</div>
      </div>
      ...
      <dl className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          ['Recorder', detail.recorder?.name ?? detail.recorder?.email ?? 'Unknown recorder'], ['Second verification', !detail.secondPersonRequired ? 'Not required' : detail.secondVerifiedAt ? `Verified ${new Date(detail.secondVerifiedAt).toLocaleString()}` : (detail.secondVerifier?.name ?? detail.secondVerifier?.email ?? 'Required, not assigned')],
        ].map(([label, value]) => <div key={label} className="rounded-xl bg-al-surface-sunken p-4"><dt className="text-xs font-black uppercase tracking-wide text-al-text-muted">{label}</dt><dd className="mt-1 text-sm font-black text-al-text">{value}</dd></div>)}
      </dl>
      <div className="rounded-xl border border-al-border bg-al-surface-sunken p-4"><p className="text-xs font-black uppercase tracking-wide text-al-text-muted">Business context</p><p className="mt-2 text-sm leading-6 text-al-text-secondary">{detail.businessContext}</p>{detail.supportingNotes ? <p className="mt-3 border-t border-al-border pt-3 text-sm leading-6 text-al-text-secondary">{detail.supportingNotes}</p> : null}</div>
      ...
      <div><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-al-accent">Supporting evidence</p><h4 className="mt-1 font-black text-al-text">Chronological associations</h4></div><span className="text-xs font-bold text-al-text-muted">{evidence.length} records</span></div><div className="mt-3 grid gap-3">{evidence.length === 0 ? <p className="rounded-xl border border-dashed border-al-border p-4 text-sm font-semibold text-al-text-muted">No supporting evidence linked or suggested yet.</p> : evidence.map((item) => <article key={item.id} className="rounded-xl border border-al-border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-al-surface-elevated px-2.5 py-1 text-xs font-black text-al-text-secondary">{display(item.origin)}</span><span className="rounded-full bg-al-info/10 px-2.5 py-1 text-xs font-black text-al-accent">{item.confidence}% match</span><span className="rounded-full bg-al-surface px-2.5 py-1 text-xs font-black text-al-text-secondary ring-1 ring-slate-200">{display(item.status)}</span></div><p className="mt-3 font-black text-al-text">{item.messageSource.provider} · {item.messageSource.channel ?? 'Source record'}</p><p className="mt-1 text-xs font-bold text-al-text-muted">{new Date(item.sourceTimestamp).toLocaleString()} · {item.messageSource.sender ?? item.messageSource.senderEmail ?? 'Unknown sender'}</p></div>{canManage && item.status === 'SUGGESTED' ? <div className="flex gap-2"><button type="button" disabled={working === item.id} onClick={() => reviewEvidence(item.id, 'CONFIRMED')} className="h-9 rounded-lg bg-emerald-600 px-3 text-xs font-black text-white">Link</button><button type="button" disabled={working === item.id} onClick={() => reviewEvidence(item.id, 'REJECTED')} className="h-9 rounded-lg border border-al-border px-3 text-xs font-black text-al-text-secondary">Reject</button></div> : null}</div><blockquote className="mt-3 rounded-lg bg-al-surface-sunken p-3 text-sm leading-6 text-al-text-secondary">{item.messageSource.excerpt}</blockquote><p className="mt-2 text-xs font-semibold text-al-text-muted">Match reasons: {item.matchingReasons.join(', ') || 'Human association'}</p>{item.rejectionReason ? <p className="mt-2 text-xs font-bold text-al-danger">Rejected: {item.rejectionReason}</p> : null}</article>)}</div></div>

      <div className="grid gap-5 xl:grid-cols-2"><div><p className="text-xs font-black uppercase tracking-wide text-al-accent">Version history</p><div className="mt-3 grid gap-2">{versions.map((version) => <div key={version.id} className="rounded-xl bg-al-surface-sunken p-4"><div className="flex justify-between gap-3"><p className="font-black text-al-text">Version {version.version}</p><p className="text-xs font-bold text-al-text-muted">{new Date(version.createdAt).toLocaleString()}</p></div><p className="mt-1 text-sm text-al-text-secondary">{version.changeReason}</p><p className="mt-1 text-xs font-bold text-al-text-muted">By {version.actorUser?.name ?? version.actorUser?.email ?? 'Unknown'}</p></div>)}</div></div><div><p className="text-xs font-black uppercase tracking-wide text-al-accent">Confirmation history</p><div className="mt-3 grid gap-2">{confirmations.length === 0 ? <p className="rounded-xl border border-dashed border-al-border p-4 text-sm font-semibold text-al-text-muted">No confirmation requests sent.</p> : confirmations.map((confirmation) => <div key={confirmation.id} className="rounded-xl bg-al-surface-sunken p-4"><div className="flex justify-between gap-3"><p className="font-black text-al-text">{display(confirmation.decision)}</p><p className="text-xs font-bold text-al-text-muted">{new Date(confirmation.respondedAt ?? confirmation.createdAt).toLocaleString()}</p></div><p className="mt-1 text-sm text-al-text-secondary">{confirmation.approverEmail}</p>{confirmation.responseNote ? <p className="mt-2 text-sm text-al-text-secondary">{confirmation.responseNote}</p> : null}</div>)}</div></div></div>
    </section>
  );
}
