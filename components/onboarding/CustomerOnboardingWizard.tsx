...
export function CustomerOnboardingWizard({ initialState }: { initialState: WizardState }) {
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(135deg,#f8fafc_0%,#eef4ff_45%,#f7f9fc_100%)] text-al-text">
      <div className="mx-auto grid min-h-screen max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[300px_1fr] lg:px-6">
        <aside className="rounded-[28px] border border-white/70 bg-al-bg p-6 text-white shadow-[0_24px_80px_rgba(15,23,42,0.18)] lg:sticky lg:top-6 lg:h-[calc(100vh-48px)]">
          <Link href="/" className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-al-accent">
              <span className="text-lg font-black">A</span>
            </span>
            <span>
              <span className="block text-xl font-black">ApprovLine</span>
              <span className="text-xs font-bold text-al-text-muted">Every approval. Captured. Proven.</span>
            </span>
          </Link>
          ...
          <header className="rounded-[28px] border border-al-border bg-al-surface/90 p-6 shadow-sm backdrop-blur">
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              <div>
                ...
                <h1 className="mt-2 text-3xl font-black tracking-tight text-al-text md:text-4xl">{onboardingStepLabels[currentKey]}</h1>
          ...
          <div className="rounded-[28px] border border-al-border bg-al-surface p-6 shadow-[0_20px_70px_rgba(15,23,42,0.08)]">
            ...
            {step === 2 ? (
              <div className="grid gap-5">
                <div className="grid gap-3 rounded-2xl border border-al-border bg-al-surface-sunken p-4 md:grid-cols-[1fr_1fr_180px_auto]">
                  ...
                  <button
                    ...
                    className="self-end rounded-xl bg-al-accent px-4 py-3 text-sm font-black text-white"
                  >
                    Add
                  </button>
                </div>
                ...
                <div className="grid gap-2">
                  {state.invitedTeamMembers.length ? state.invitedTeamMembers.map((invite, index) => (
                    <div key={`${invite.email}-${index}`} className="flex items-center justify-between rounded-xl border border-al-border p-3">
                      <div>
                        <p className="font-black text-al-text">{invite.name || invite.email}</p>
                        <p className="text-sm text-al-text-muted">{invite.email} · {invite.role}</p>
                      </div>
                  ...
                  )) : <p className="rounded-xl bg-al-surface-sunken p-4 text-sm font-semibold text-al-text-muted">No team members drafted yet.</p>}
                </div>
              </div>
            ) : null}
            ...
            {step === 4 ? (
              <div className="grid gap-5">
                ...
                <div className="flex gap-2">
                  <input value={customCategory} onChange={(event) => setCustomCategory(event.target.value)} placeholder="Custom approval category" className="h-11 flex-1 rounded-xl border border-al-border px-4 text-sm font-bold outline-none focus:border-al-accent" />
                  <button type="button" onClick={() => {
                    if (!customCategory.trim()) return;
                    update((draft) => ({ ...draft, approvalCategories: [...new Set([...draft.approvalCategories, customCategory.trim()])] }));
                    setCustomCategory('');
                  }} className="rounded-xl bg-al-accent px-4 text-sm font-black text-white">Add</button>
                </div>
              </div>
            ) : null}
            ...
            {step === 5 ? (
              <div className="grid gap-3 md:grid-cols-2">
                {state.integrationSetup.map((integration) => (
                  <div key={integration.provider} className="rounded-2xl border border-al-border p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-lg font-black text-al-text">{integration.provider}</p>
                        <p className="mt-1 text-sm text-al-text-muted">Customer IT/Admin owns OAuth connection.</p>
                      </div>
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-2">
                      {(['Connected', 'Skipped', 'Requires Attention'] as const).map((status) => (
                        <button key={status} type="button" onClick={() => update((draft) => ({ ...draft, integrationSetup: draft.integrationSetup.map((item) => item.provider === integration.provider ? { ...item, status } : item) }))} className="rounded-lg border border-al-border px-2 py-2 text-xs font-black text-al-text-secondary hover:bg-al-surface-sunken">{status}</button>
                      ))}
                    </div>
                  </div>
                ))}
            ...
            {step === 6 ? (
              <div className="grid gap-4">
                <div className="rounded-2xl border border-dashed border-al-border-strong bg-al-surface-sunken p-6 text-center">
                  <p className="text-lg font-black text-al-text">Upload policies and SOPs</p>
                  <p className="mt-2 text-sm text-al-text-muted">PDF, DOCX, TXT, and Markdown are supported. This wizard records readiness; use Playbook AI for full ingestion.</p>
                  <input
                    type="file"
                    multiple
                ...
                <div className="grid gap-2">
                  {state.playbookSetup.map((playbook, index) => (
                    <div key={`${playbook.name}-${index}`} className="rounded-xl border border-al-border p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="font-black text-al-text">{playbook.name}</p>
                          <p className="text-sm text-al-text-muted">{playbook.category}</p>
                        </div>
            ...
            {step === 7 ? (
              <div className="grid gap-5">
                <p className="text-sm leading-6 text-al-text-secondary">Initialize baseline entities for departments, users, policies, and approval categories.</p>
                <div className="grid gap-3 md:grid-cols-4">
                  {['Departments', 'Users', 'Policies', 'Approval Categories'].map((item) => <div key={item} className="rounded-2xl border border-al-border p-4 text-sm font-black">{item}</div>)}
                </div>
                <button type="button" onClick={() => update((draft) => ({ ...draft, memoryGraphInitializedAt: draft.memoryGraphInitializedAt ?? new Date().toISOString() }))} className="w-fit rounded-xl bg-al-accent px-5 py-3 text-sm font-black text-white">
            ...
            {step === 8 ? (
              <div className="grid gap-5">
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  {sampleQuestions.map((question) => <div key={question} className="rounded-xl border border-al-border bg-al-surface-sunken p-3 text-sm font-bold text-al-text-secondary">{question}</div>)}
                </div>
              </div>
            ) : null}
            ...
            {step === 9 ? (
              <div className="grid gap-3">
                <p className="text-5xl font-black text-al-text">{readinessScore}/100</p>
                {[
                ...
                ].map(([label, ok]) => <div key={String(label)} className="flex items-center justify-between rounded-xl border border-al-border p-3"><span className="font-bold">{label}</span><Badge tone={ok ? 'green' : 'amber'}>{ok ? 'Ready' : 'Needs setup'}</Badge></div>)}
              </div>
            ) : null}

            {step === 10 ? (
              <div className="grid gap-6">
                <div className="rounded-3xl bg-al-bg p-8 text-white">
                  ...
                  <p className="mt-3 text-sm leading-6 text-al-text-secondary">Your workspace is ready for approvals, investigations, Copilot, Playbook AI, Memory Graph, and Executive ROI.</p>
                </div>
                <div className="grid gap-3 md:grid-cols-3">
                  {['Executive Dashboard', 'Investigation Center', 'AI Copilot', 'Playbook AI', 'Memory Graph', 'Integrations'].map((item) => <div key={item} className="rounded-2xl border border-al-border p-4 text-sm font-black">{item}</div>)}
                </div>
              </div>
            ) : null}
          </div>
          ...
          <footer className="flex flex-col-reverse justify-between gap-3 rounded-[24px] border border-al-border bg-al-surface p-4 shadow-sm sm:flex-row sm:items-center">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {step < 10 ? (
                ...
                <button type="button" onClick={() => save(10, 'go-live', true)} disabled={isPending} className="rounded-xl bg-al-accent px-5 py-3 text-sm font-black text-white shadow-sm shadow-blue-200 disabled:opacity-50">Launch Workspace</button>
              )}
            </div>
          </footer>
        </section>
