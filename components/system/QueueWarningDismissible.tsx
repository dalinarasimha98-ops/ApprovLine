...
export function QueueWarningDismissible({ message }: { message?: string }) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 shadow-sm">
      <div>
        <p className="font-bold">Background processing is temporarily unavailable.</p>
        <p className="mt-1 text-amber-800">
          Core onboarding and approval tracking continue to work.
          {message ? <span className="text-amber-700"> {message}</span> : null}
        </p>
      </div>
      <button
        type="button"
