...
export async function extractPlaybookText(file: File) {
  ...
  if (extension === "txt" || extension === "md" || file.type.includes("text")) {
    return stripBinaryNoise(buffer.toString("utf8"));
  }
  ...
  if (extension === "pdf") {
    ...
    try {
      ...
      return stripBinaryNoise(result.text);
    } finally {
  ...
  if (extension === "docx") {
    ...
    return stripBinaryNoise(result.value);
  }

  throw new Error("Unsupported playbook document format.");
}
...
function inferCategory(text: string, fallback?: string) {
  ...
  if (/\b(contract|msa|legal|redline|liability|indemnity|terms)\b/.test(lower))
    return "Legal";
  if (/\b(procurement|vendor|supplier|purchase|po|payment terms)\b/.test(lower))
    return "Procurement";
  if (/\b(finance|budget|spend|invoice|payment|cost center|cfo)\b/.test(lower))
    return "Finance";
  if (
    /\b(security|soc 2|access|production|vulnerability|subprocessor)\b/.test(
      lower,
    )
  )
    return "Security";
  if (/\b(compliance|gdpr|audit|retention|governance|control)\b/.test(lower))
    return "Compliance";
  if (/\b(hr|candidate|compensation|employee|offer)\b/.test(lower)) return "HR";
  if (/\b(engineering|release|code|deploy|architecture)\b/.test(lower))
    return "Engineering";
...
function amountThreshold(text: string) {
  const matches = [
    ...text.matchAll(/\$?\s?([0-9]{1,3}(?:,[0-9]{3})+|[0-9]+)\s?(?:k|K|000)?/g),
  ];
...
function evidenceFromText(text: string) {
  ...
  if (lower.includes("business justification"))
    evidence.add("Business justification");
  if (lower.includes("signature authority"))
    evidence.add("Signature authority");
  return [...evidence];
}
...
function severityForRule(
  text: string,
  spendingLimit?: number,
): ExtractedPlaybookRule["severity"] {
  const lower = text.toLowerCase();
  if (
    lower.includes("critical") ||
    (spendingLimit && spendingLimit >= 100000) ||
    lower.includes("production access")
  )
    return "critical";
  if (
    lower.includes("security") ||
    lower.includes("legal") ||
    (spendingLimit && spendingLimit >= 25000)
  )
    return "high";
...
export function extractPlaybookRules(
  content: string,
  category?: string,
  ...
  chunks.forEach((chunk, index) => {
    const sentences = chunk
      ...
      .map((item) => item.trim())
      .filter(Boolean);
    const ruleSentences = sentences.filter((sentence) =>
      /\b(require|required|must|approval|approv|escalat|above|over|greater than|evidence|review)\b/i.test(
        sentence,
      ),
    );
    const source =
      ruleSentences.join(" ").slice(0, 1200) || chunk.slice(0, 1200);
    ...
    const requiredDepartments = requiredApprovers.length
      ? requiredApprovers.filter(
          (item) => !["CFO", "Manager", "Department Head"].includes(item),
        )
      : [inferredCategory];
    const evidenceRequired = evidenceFromText(source);
    const escalationChain =
      source.toLowerCase().includes("escalat") ||
      source.toLowerCase().includes("cfo")
        ? ["Manager", "Department Head", "CFO"].filter(
            (item) =>
              source.toLowerCase().includes(item.toLowerCase()) ||
              item === "CFO",
          )
        : [];
    const titleAmount = spendingLimit
      ? ` above $${new Intl.NumberFormat("en-US").format(spendingLimit)}`
      : "";
    ...
    rules.push({
      ...
      requiredApprovers: requiredApprovers.length
        ? requiredApprovers
        : [`${inferredCategory} approval`],
      requiredDepartments: requiredDepartments.length
        ? requiredDepartments
        : [inferredCategory],
      escalationChain,
      spendingLimit,
      riskTriggers: [
        ...(source.toLowerCase().includes("customer data")
          ? ["Customer data access"]
          : []),
        ...(source.toLowerCase().includes("production")
          ? ["Production system access"]
          : []),
        ...(source.toLowerCase().includes("non-standard")
          ? ["Non-standard terms"]
          : []),
      ],
      evidenceRequired: evidenceRequired.length
        ? evidenceRequired
        : ["Approval evidence", "Source message link"],
...
export async function indexPlaybookDocument(input: {
  ...
  try {
    ...
    const rules = extractPlaybookRules(
      input.content,
      input.category ?? String(input.metadata?.category ?? ""),
    );
...
function approvalAmount(
  approval: Pick<
    ApprovalRecord,
    "subject" | "businessImpact" | "evidenceSnippet" | "reasoning"
  >,
) {
...
function ruleAppliesToApproval(
  rule: PlaybookRule,
  approval: Pick<
    ApprovalRecord,
    | "subject"
  ...
  const categoryMatch = [approval.department, approval.category]
    .filter(Boolean)
    .some((item) => item?.toLowerCase() === rule.category.toLowerCase());
  const keywordMatch = [
    rule.category,
    ...rule.requiredDepartments,
    ...rule.riskTriggers,
  ].some((item) => combined.includes(item.toLowerCase()));
  const thresholdMatch = rule.spendingLimit
    ? Boolean(amount && amount >= rule.spendingLimit)
    : false;
  return categoryMatch || keywordMatch || thresholdMatch;
}

function approverPresent(
  required: string,
  approval: Pick<
    ApprovalRecord,
    | "approverName"
...
function evidencePresent(
  ...
  approval: Pick<
    ApprovalRecord,
    "evidenceSnippet" | "sourceLink" | "reasoning" | "conditions"
  >,
) {
  const combined = [
    approval.evidenceSnippet,
...
export async function evaluateApprovalCompliance(
  organizationId: string,
  approvalId: string,
) {
  ...
  const applicable = rules.filter((rule) =>
    ruleAppliesToApproval(rule, approval),
  );
  const rule = applicable[0] ?? rules[0] ?? null;

  const missingApprovers = rule
    ? rule.requiredApprovers.filter((item) => !approverPresent(item, approval))
    : [];
  const missingDepartments = rule
    ? rule.requiredDepartments.filter(
        (item) => !approverPresent(item, approval),
      )
    : [];
  const missingEscalationSteps = rule
    ? rule.escalationChain.filter((item) => !approverPresent(item, approval))
    : [];
  const missingEvidence = rule
    ? rule.evidenceRequired.filter((item) => !evidencePresent(item, approval))
    : [];
  ...
  await prisma.approvalComplianceEvaluation.deleteMany({
    where: { organizationId, approvalRecordId: approval.id },
  });
...
export async function evaluateRecentApprovals(
  organizationId: string,
  limit = 50,
) {
  ...
  for (const approval of approvals) {
    const result = await evaluateApprovalCompliance(
      organizationId,
      approval.id,
    );
...
export async function getPlaybookComplianceInsights(organizationId: string) {
  ...
  return {
    ...
    partial,
    nonCompliant,
    mostViolatedPolicies: violationsByRule.map((item) => ({
      name: item.triggeredRule ?? "Unmatched rule",
      count: item._count._all,
    })),
    departmentsWithHighestViolations: [...departmentMap.entries()]
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 6),
    riskTrend: violationsByDepartment.map((item) => ({
      name: item.severity,
      count: item._count._all,
    })),
...
export async function searchPlaybookChunks(
  organizationId: string,
  question: string,
  limit = 5,
): Promise<SourceMatch[]> {
...
function inferGuidance(
  ...
  if (
    /\b(\$|budget|spend|payment|invoice|finance|amount|cost)\b/.test(combined)
  ) {
  ...
  if (
    /\b(security|soc 2|data|access|pii|gdpr|compliance|risk)\b/.test(combined)
  ) {
  ...
  const confidence = Math.max(
    ...
    Math.min(
      96,
      Math.round(78 + (sources[0]?.score ?? 0) * 18 + sources.length * 2),
    ),
  );
  const compliant =
    sources.length === 0
  ...
  try {
    ...
    return {
      ...
      requiredApprovers: Array.isArray(parsed.requiredApprovers)
        ? parsed.requiredApprovers.map(String)
        : [],
      requiredDepartments: Array.isArray(parsed.requiredDepartments)
        ? parsed.requiredDepartments.map(String)
        : [],
      policySections: Array.isArray(parsed.policySections)
        ? (parsed.policySections as PlaybookAnswer["policySections"])
        : [],
      evidenceMissing: Array.isArray(parsed.evidenceMissing)
        ? parsed.evidenceMissing.map(String)
        : [],
      compliant:
        parsed.compliant === "yes" || parsed.compliant === "no"
          ? parsed.compliant
          : "needs_review",
...
export async function queryPlaybooks(input: {
  organizationId: string;
  actorUserId?: string;
  question: string;
}) {
  const sources = await searchPlaybookChunks(
