...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error('[evidence-suggestions] correlation failed', error);
    return NextResponse.json({ error: 'Evidence correlation is temporarily unavailable. Retry in a moment.' }, { status: 503 });
  }
}
